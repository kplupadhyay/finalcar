                            ...
                            <tbody>
                                @forelse($countries as $country)
                                    <tr>
                                        ...
                                        <td>
                                            <a href="{{ route('admin.carlocation.cit.all') }}?search={{ $country->name }}"><span class="badge badge--primary">{{ $country->total_city }}</span></a>
                                        </td>
                                        ...
                                        <td>
                                            <div class="button-group">
                                                ...
                                                @if ($country->status == Status::DISABLE)
                                                    <button class="btn btn-sm btn-outline--success me-1 confirmationBtn" data-action="{{ route('admin.carlocation.countr.status.update', $country->id) }}" data-question="@lang('Are you sure to enable this facility?')" type="button">
                                                        <i class="la la-eye"></i> @lang('Enable')
                                                    </button>
                                                @else
                                                    <button class="btn btn-sm btn-outline--danger confirmationBtn" data-action="{{ route('admin.carlocation.countr.status.update', $country->id) }}" data-question="@lang('Are you sure to disable this facility?')" type="button">
    ...
    <div id="cuModal" class="modal fade" tabindex="-1" role="dialog">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form action="{{ route('admin.carlocation.countr.add') }}" method="POST">
                    @csrf
