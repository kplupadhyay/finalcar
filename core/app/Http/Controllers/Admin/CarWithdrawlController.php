...
use App\Models\CarTransactionLog;
use App\Models\CarWithdrawlLog;
use Illuminate\Http\Request;

class CarWithdrawalController extends Controller
{
    ...
    {
        if ($scope) {
            $withdrawals = CarWithdrawlLog::$scope();
        } else {
            $withdrawals = CarWithdrawlLog::where('status', '!=', Status::PAYMENT_INITIATE);
        }
    ...
    {
        $withdrawal = CarWithdrawlLog::where('id', $id)->where('status', '!=', Status::PAYMENT_INITIATE)->with(['owner', 'method'])->firstOrFail();
    ...
    public function approve(Request $request)
    {
        ...
        $withdraw = CarWithdrawlLog::where('id', $request->id)->where('status', Status::PAYMENT_PENDING)->with('owner', 'method')->firstOrFail();
    ...
    {
        $request->validate(['id' => 'required|integer']);
        $withdraw = CarWithdrawlLog::where('id', $request->id)->where('status', Status::PAYMENT_PENDING)->with('owner')->firstOrFail();
        ...
        $transaction = new CarTransactionLog();
