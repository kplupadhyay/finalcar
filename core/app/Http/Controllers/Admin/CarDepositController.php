...
use App\Models\CarGateway;
use App\Models\CarBooking;
use App\Http\Controllers\Controller;
use App\Http\Controllers\Gateway\PaymentController;
use Illuminate\Http\Request;

class CarDepositController extends Controller
{
    public function pending()
    {
        $pageTitle = 'Pending Payments';
        $deposits = $this->depositData('pending');
        return view('admin.cardeposit.carlog', compact('pageTitle', 'deposits'));
    }
    ...
    public function deposit()
    {
        ...
        return view('admin.cardeposit.carlog', compact('pageTitle', 'deposits', 'successful', 'pending', 'rejected', 'initiated'));
    }

    protected function depositData($scope = null, $summery = false)
    {
        if ($scope) {
            $deposits = CarDeposit::$scope()->with(['user', 'gateway', 'owner']);
        } else {
            $deposits = CarDeposit::with(['user', 'gateway', 'owner']);
        }
        ...
        //vai method
        if ($request->method) {
            $method = CarGateway::where('alias', $request->method)->firstOrFail();
    ...
    {
        $deposit = CarDeposit::where('id', $id)->with(['user', 'gateway', 'owner'])->firstOrFail();
        ...
        return view('admin.cardeposit.cardetail', compact('pageTitle', 'deposit', 'details'));
    }


    public function approve($id)
    {
        $deposit = CarDeposit::where('id', $id)->where('status', Status::PAYMENT_PENDING)->firstOrFail();

        if ($deposit->booking_id) {
            $booking = CarBooking::find($deposit->booking_id);
        ...
        return to_route('admin.cardeposit.pending')->withNotify($notify);
    }
    ...
    public function reject(Request $request)
    {
        ...
        $deposit = CarDeposit::where('id', $request->id)->with('user', 'owner')->where('status', Status::PAYMENT_PENDING)->firstOrFail();
